...
def predict():
    ...
    df_2['tenure_group'] = pd.cut(df_2.tenure.astype(int), range(1, 80, 12), right=False, labels=label)
    #drop column customerID and tenure  
    df_2 = df_2.drop(['tenure'], axis=1, inplace=True) 
    
    df2_cat = df_2[['gender', 'Partner', 'Dependents', 'PhoneService',
           'MultipleLines', 'InternetService', 'OnlineSecurity', 'OnlineBackup',
           'DeviceProtection', 'TechSupport', 'StreamingTV', 'StreamingMovies',
           'Contract', 'PaperlessBilling', 'PaymentMethod','tenure_group']]
    
    new_df__dummies = pd.get_dummies(df2_cat, dtype='int')
    
    new_df_num = df_2[['SeniorCitizen', 'MonthlyCharges', 'TotalCharges']]
    final_df=pd.concat([new_df__dummies, new_df_num], axis=1)
    ...
    return render_template('template.html', output1=o1, output2=o2, 
